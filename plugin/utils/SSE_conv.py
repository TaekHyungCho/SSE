...
class SEPConv_H_H(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, basis, odd_mask, even_mask, scale_size, kernel_size,
        ...
        if self.scale_size > 1:
            self.weight_1x1 = nn.Parameter(torch.Tensor(out_channels, in_channels, scale_size, 1, 1))
        else:
            self.weight_1x1 = nn.Parameter(torch.Tensor(out_channels, in_channels, 1, 1))
        if bias:
            self.bias_LF = nn.Parameter(torch.Tensor(out_channels))
            self.bias_HF = nn.Parameter(torch.Tensor(out_channels))
        else:
    ...
    def forward(self,x):
        ...
        if self.res:
            ...
            out_hf = self.conv_1x1(x_hf,self.weight_1x1)
